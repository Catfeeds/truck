...
export default class TouristTagModify extends Component{
  constructor(props) {
    super(props);
  ...
  componentWillMount(){
    ...
    Tool.fetch(null,`${config.appUrlPath}rest/tag/travelertags`,'get',null,(ret)=>{
      this.setState({list: ret || []},()=>{
         this.changeParent(1001);
      });
    });
    
  }
  save(){
    let {tagIds, tagIdsOld} = this.state;
    ...
    if(tagIds.length > 0){
      ...
      tagIdsOld.map( (vn, kn)=>{
        ...
        for (var i = 0; i < tagIds.length; i++) {
          if( vn == tagIds[i])isDel = false;
        }
        if(isDel)del.push(vn)
      });
    }

    Tool.fetch(null,`${config.appUrlPath}rest/cust`,'put',{ids:del, tagIds:add },(ret)=>{
      Tool.alert('信息跟新成功')
    });
