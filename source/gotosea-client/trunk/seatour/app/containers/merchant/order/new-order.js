		...
		return (
			<View style = { orderStyles.item } >
				<TouchableOpacity style = { orderStyles.itemContent } onPress={()=>Tool.to('merchantOrderDetail')}>
					...
					<ContentRow title = {'创建时间:'} text = '8686868' />
				</TouchableOpacity>
				<BorderLine/>
				<View style = { orderStyles.tool }>
					<View style = { orderStyles.toolLeft }>
						<Text style = { orderStyles.priceTitle }>订单总价：<Text  style = { orderStyles.price } >¥456</Text></Text>
					</View>
					<TouchableOpacity style = { orderStyles.toolRight }>
						<Text style = { orderStyles.confirmText}>确认订单</Text>
					</TouchableOpacity>
				</View>
				<BorderLine/>
			</View>
